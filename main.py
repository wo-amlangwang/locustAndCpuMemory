...
import logging
import threading
from Utils.singleton import set_access_log
...
def config_argument_parser():
    ...
    parser.add_argument('--collect_time_in_second', help='Total time to run', default=60, type=int)
    parser.add_argument('--access_log', help='access log to config locust', default=get_default_access_log())
...
def get_default_access_log():
    return f'{os.path.dirname(os.path.realpath(__file__))}{get_slash()}access.log'
...
if __name__ == '__main__':
    ...
    logger = logging.getLogger(__name__)
    set_access_log(str(args.access_log))
    ...
    try:
        ...
        pass
    locust_task = threading.Thread(run_locust, args=(locust_runner, args))
    process_monitor_task = threading.Thread(run_process_monitor, args=(process_monitor, args))
    locust_task.run()
    process_monitor_task.run()
    locust_task.join()
    process_monitor_task.run()
